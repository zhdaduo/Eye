...
class ScrollCalculatorHelper(private val playId: Int, private val rangeTop: Int, private val rangeBottom: Int) {
    ...
    private fun showWifiDialog(gsyBaseVideoPlayer: GSYBaseVideoPlayer, context: Context) {
        ...
        builder.setPositiveButton(context.resources.getString(com.shuyu.gsyvideoplayer.R.string.tips_not_wifi_confirm)) { dialog, _ ->
            dialog.dismiss()
            gsyBaseVideoPlayer.startPlayLogic()
        }
        builder.setNegativeButton(context.resources.getString(com.shuyu.gsyvideoplayer.R.string.tips_not_wifi_cancel)) { dialog, _ -> dialog.dismiss() }
        builder.create().show()
    }
}
