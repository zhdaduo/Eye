            ...
            binding.root.nine_image.apply {
                ...
                setItemDelegate(object : ImageNice9Layout.ItemDelegate {
                    override fun onItemClick(position: Int) {
                        ...
                        callback?.onPictureClickListener(position, item?.data?.content?.data?.urls as ArrayList<String>,
                                Photo(ownerBean.avatar, ownerBean.nickname, item?.data?.content?.data?.description!!,
                                        false, consumptionBean.collectionCount, consumptionBean.replyCount, tagGroupList?.toList()!!))
                    }
                })
            }
            binding.root.nine_image.outlineProvider = CornerOutlineProvider()
            binding.root.nine_image.clipToOutline = true
        }
