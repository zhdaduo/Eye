...
class RetrofitException(
    ...
    private var _errorData: ErrorResponse? = null
    ...
    fun getErrorData(): ErrorResponse? = _errorData

    private fun deserializeErrorResponse() {
        if (_response != null && _response.errorBody() != null) {
            try {
                _errorData = getErrorBodyAs(ErrorResponse::class.java)
            } catch (e: IOException) {
                Log.e("Error deserialization", e.toString())
            }
        }
    }
